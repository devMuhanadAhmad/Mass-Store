<x-dashboard>
    <div class="col-12 grid-margin stretch-card ">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">{{ __('Edit Role User') }}</h4>
                <form action="{{ route('roleuser.update', $roleuser->id) }}" method="post" enctype="multipart/form-data"
                    class="forms-sample">
                    @csrf
                    @method('put')
                    @include('admin.roleuser._form')
                </form>
            </div>
        </div>
    </div>
